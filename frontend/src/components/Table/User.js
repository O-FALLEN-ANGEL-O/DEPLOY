...
function User() {
    ...
    async function handleAddUser() {
        try {
            await axios.post("http://localhost:8801/users", formData);
            handleCloseModal();
    ...
    async function handleUpdateUser() {
        try {
            await axios.put(`http://localhost:8801/users/${formData.id}`, formData);
            handleCloseModal();
    ...
    async function handleDelete(id) {
        try {
            ...
            if (confirmDelete) {
                await axios.delete(`http://localhost:8801/users/${id}`);
                fetchUsers();
