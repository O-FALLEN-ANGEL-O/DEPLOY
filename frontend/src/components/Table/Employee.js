...
function Employee() {
    ...
    async function fetchUsers() {
        try {
            const response = await axios.get("http://localhost:8801/EmpDetails");
    ...
    async function handleAddUser() {
        try {
            await axios.post("http://localhost:8801/EmpDetails", formData);
            handleCloseModal();
    ...
    async function handleUpdateUser() {
        try {
            await axios.put(`http://localhost:8801/EmpDetails/${formData.id}`, formData);
            handleCloseModal();
    ...
    async function handleDelete(id) {
        try {
            ...
            if (confirmDelete) {
                await axios.delete(`http://localhost:8801/EmpDetails/${id}`);
                fetchUsers();
        ...
        <div>
            ...
            <Modal show={showModal} onHide={handleCloseModal}>
                <Modal.Header closeButton>
                    <Modal.Title>{modalMode === 'add' ? 'Add Employee' : modalMode === 'edit' ? 'Edit Employee' : 'View Employee'}</Modal.Title>
                </Modal.Header>
                <Modal.Body>
