...
function LeadTable() {
    ...
    async function fetchLeads() {
        try {
            const response = await axios.get("http://localhost:8801/Leads");
    ...
    async function handleAddLead() {
        try {
            await axios.post("http://localhost:8801/Leads", formData);
            handleCloseModal();
    ...
    async function handleUpdateLead() {
        try {
            await axios.put(`http://localhost:8801/Leads/${formData.id}`, formData);
            handleCloseModal();
    ...
    async function handleDelete(id) {
        try {
            ...
            if (confirmDelete) {
                await axios.delete(`http://localhost:8801/Leads/${id}`);
                fetchLeads();
